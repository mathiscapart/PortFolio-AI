"""API FastAPI du portfolio : health check et boucle vocale (`/voice`).

Pas d'endpoint texte en production : pour interroger le RAG à l'écrit, voir
`backend/tests/question_texte.py`, hors du code servi.
...
# l'API reste importable sans la pile GPU, ce qui permet de tester /health
# en CI sur un runner standard.
...
# "ou a-t-elle fait ses etudes" arrive 6e. A k=5 il tombait hors contexte et
# l'assistant refusait -- correctement, mais a tort.
K_EXTRAITS = 8

# Passage d'identité toujours transmis au LLM. Mesuré en prod : sur une question
# vague (« dis-moi un peu plus sur ton parcours »), la recherche ne ramenait que
# GitHub et Piloti, et le LLM inventait « développeur full-stack » 5 fois sur 5.
IDENTITE_SOURCE = "parcours.md"
IDENTITE_TITRE = "Mon parcours en bref"


def ajouter_identite(chunks: list[dict], identite: dict | None) -> list[dict]:
    """Place le passage d'identité en tête des extraits s'il n'y est pas déjà.
    Sans passage (corpus de démonstration, section renommée), rien ne change."""
    if identite is None:
        return chunks
    if any(c["source"] == identite["source"] and c.get("titre") == identite.get("titre") for c in chunks):
        return chunks
    return [{**identite, "score": None}, *chunks]
...
async def _gerer_session_vocale(websocket: WebSocket):
    ...
    try:
        chunks = await run_in_threadpool(
            qdrant_store.search, settings.qdrant_collection, question, embedding_model, K_EXTRAITS
        )
        identite = await run_in_threadpool(
            qdrant_store.get_chunk, settings.qdrant_collection, IDENTITE_SOURCE, IDENTITE_TITRE
        )
        chunks = ajouter_identite(chunks, identite)
